...
class LoginEstudante extends Component {
    ...
    teste(e){
        ...
        if(email === "karina" && senha === "karina123"){
            this.setState({redirectTo : "d8cFJ8wFcEklh8GN8Ad21"});
            invalido = false;
        }

        if(email === "suzzane" && senha === "suzzane123"){
            this.setState({redirectTo : "l8dNS4wDfVwsa7WD8Ac22"});
            invalido = false;
        }

        if(email === "kleuber" && senha === "kleuber123"){
            this.setState({redirectTo : "p8cNG6eACdFdh8FW6Dc87"});
            invalido = false;
        }

        if(email === "erivaldo" && senha === "erivaldo123"){
            this.setState({redirectTo : "w7dSS5wSzBfeI7Wv8dS31"});
            invalido = false;
        }

        if(email === "ray" && senha === "ray123"){
            this.setState({redirectTo : "x2cFS6wVxSdhi7SM7Da97"});
            invalido = false;
        }

        if(email === "ernane" && senha === "ernane123"){
            this.setState({redirectTo : "p7cSA8wxVGaiT8WHdSc32"});
            invalido = false;
        }

        if(email === "joao" && senha === "joao123"){
            this.setState({redirectTo : "j7dGD8qFxBgdh8PN7Wd24"});
            invalido = false;
        }

        if(email === "pedro" && senha === "pedro123"){
            this.setState({redirectTo : "c8sNG5qVcGgdh0QE5Td22"});
            invalido = false;
        }

        if(email === "gessica" && senha === "gessica123"){
            this.setState({redirectTo : "p8sWK5eDjFqao8QW4Cg82"});
            invalido = false;
        }

        if(email === "hayranne" && senha === "hayranne123"){
            this.setState({redirectTo : "o5fEA8qHDCfaw7WT4Fs22"});
            invalido = false;
        }
